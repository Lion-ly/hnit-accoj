...
def cal_sum_formula(sheet_infos, list_infos, sheets_len, borrow_or_lend, period, subject_name):
    ...
    for i in range(0, sheets_len):
        ...
        if subject in list_infos:
            sum += sheet_info[key_name]
    # 将结果存入
    sum = round(sum, 2)
    profit_statement_infos[subject_name][period] = sum
    return sum
...
def cal_profit_statement(company):
    ...
    # 计算每期的营业收入
    operate_income_last = cal_sum_formula(balance_sheet_infos_1,profits, len_sheet_1, borrow_or_lend=False, period="period_last", subject_name="营业收入")
    operate_income_end = cal_sum_formula(balance_sheet_infos_2, profits, len_sheet_2, borrow_or_lend=False, period="period_end", subject_name="营业收入")

    # 计算营业成本 主营业务成本  其他业务成本
    main_cost_list = ["主营业务成本", "其他业务成本"]
    main_cost_last = cal_sum_formula(balance_sheet_infos_1, main_cost_list, len_sheet_1, borrow_or_lend=True, period="period_last", subject_name="营业成本")
    main_cost_end = cal_sum_formula(balance_sheet_infos_2, main_cost_list, len_sheet_2, borrow_or_lend=True, period="period_end", subject_name="营业成本")
    ...
    # 计算投资收益
    B_lists = ["投资收益"]
    invest_income_last = cal_formula(balance_sheet_infos_1, B_lists, len_sheet_1, period="period_last", borrow_lend=False)
    invest_income_end = cal_formula(balance_sheet_infos_2, B_lists, len_sheet_2, period="period_end", borrow_lend=False)
    invest_income_last -= cal_formula(balance_sheet_infos_1, B_lists, len_sheet_1, period="period_last", borrow_lend=True)
