...
//===============================================事件控制===============================================//
/**
 * 将用户输入数据改变事件`处理函数`绑定到input
 */
function bindControlViii() {
    let inputs = $("#viiiSecond").find("input");
...
function eventChangeViii(obj) {
    ...
    //  判断投资收益是否为空
    let $select1 = $("#viiiSecond").find("input[name^=投资收益]");
    $.each($select1, function (index, item) {
        if ($(item).val()) {
            // 不为空，计算营业利润
            let name = $(item).attr("name"),
                isEnd = name.endsWith("End"),
                inputName = isEnd ? "营业利润End" : "营业利润Last",
    ...
    //  判断营业外支出是否为空
    let $select2 = $("#viiiSecond").find("input[name^=营业外支出]");
    $.each($select2, function (index, item) {
        if ((!index && $(item).val() && flag1) || (index && $(item).val() && flag2)) {
            // 不为空，计算利润总额
            let name = $(item).attr("name"),
                isEnd = name.endsWith("End"),
    ...
    //  判断所得税费用是否为空
    let $select3 = $("#viiiSecond").find("input[name^=所得税费用]");
    $.each($select3, function (index, item) {
        if ((!index && $(item).val() && flag1 === 2) || (index && $(item).val() && flag2 === 2)) {
            // 不为空，计算净利润
            let name = $(item).attr("name"),
                isEnd = name.endsWith("End"),
